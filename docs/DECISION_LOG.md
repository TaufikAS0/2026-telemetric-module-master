...
## D-027 — Machine-readable QC summary and the v0.6.2 Hardware QC release

- Status: Accepted (draft; compile evidence only, physical QC pending)
- Evidence: v0.6.0 introduced the six mandatory QC items (D-012/D-015/D-016/D-017/D-020/D-021) and v0.6.1 corrected the artifact geometry (D-026), but `/api/status` exposed each test with bespoke field names, so the portal/library could not read a normalized per-item state without parsing every block. The user directed (2026-09-04) a new Hardware QC release based on the v0.6.0/v0.6.1 QC functions.
- Decision: v0.6.2 keeps every proven QC capability and adds a read-only `"qcSummary"` block to `/api/status` (plus top-level `productCode`/`hardwareRevision`). Each of the six items (`led`, `buttons`, `aht10`, `ethernet`, `sd`, `rs485`) carries a state derived ONLY from the existing state structures: `untested` (belum ada bukti live), `testing` (tes berjalan), `pass` (bukti live otomatis lengkap dan segar), `fail` (tes berakhir gagal), `manual` (butuh keputusan operator). `led` and `buttons` are marked `manualRequired: true` and can never leave `manual` by automation — the visual verdict stays a human decision. The simulator contract mirror lives in `simulator/qc-summary.mjs` with regression tests in `tests/qc-summary.test.mjs`.
- PASS boundary: nothing in this change converts compile success or live automated evidence into a hardware PASS. PASS/BYPASS remain operator decisions recorded in the QC portal (D-012), and the exported QC JSON is still the only hardware evidence record. Compile/test success is not QC PASS.
- Release metadata: the package is released as `firmwareRole: "qc"`, `stage: "lab"`, `lifecycle: "draft"`, canonical `releaseId TMM-<version>-<buildId>`, one merged/full BIN (USB) + one app-only BIN (OTA) from the same commit. No partition change: the board-proven `tmm-ota-4mb` table (D-024/D-026) is byte-identical, so app-only OTA stays compatible with v0.6.x devices; USB merged flash remains the first-install/recovery path. No pins, sensors, protocols, or boot-time outputs were added or changed.
- Unknown: the new summary surface and both BINs on real hardware (portal rendering, OTA upgrade from v0.6.1, rollback, physical QC run) remain pending physical verification.
